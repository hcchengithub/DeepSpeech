...
peforth.ok(loc=locals(),glo=globals(),cmd='''
    :> value main // ( -- {loc,glo,prompt} ) main program variables
    main :> [0] value main.locals // ( -- dict ) main locals
    : sh py> os.system(pop()) ; // ( "command line" -- errorlevel ) Bash shell command line
    \ Check ffmpeg, ffprobe, and the needed '~/GitHub/DeepSpeech/native_client/audio' directory
        ...
        s" ls ../audio" sh \ 預設在 WSL Ubuntu 的 ~/GitHub/DeepSpeech/native_client/python 執行 
...
def predict(msg):
    ...
    results += modify me for deepspeech ai.predict("1.png")
